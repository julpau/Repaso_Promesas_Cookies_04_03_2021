...
window.onload = function(){
    const data = {mes : 1, dia : 7, ano : 2022, hora : 0, min : 1, seg : 10 };
    ...
    const Cookie = new Object({
        ...
        Dia : function(id){
            ...
            const calendar = new Date(this.Ano(), (dia+1<=12 && dia+1>=1) ? dia+1 : dia-1, 0);
            return (id<=calendar.getDate()) ? (id>=this.date.getDay()) ? "0"+id : this.date.getDay() : this.date.getDay();
        },
        Ano : function(id=0){
    ...
    document.cookie = "Nombre=Hersain; expires="+Cookie.limite(data)+";";
    var date = new Date();
